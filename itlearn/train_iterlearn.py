...
from agents_utils import eval_model, valid_model, eval_fr_en_stats
import os
...
def itlearn_loop(args, model, train_it, dev_it, extra_input, loss_cos, loss_names, monitor_names):
    ...
    for iters, train_batch in enumerate(train_it):
        ...
        if (iters + 1) % args.generation_steps == 0:
            ...
            student.train()
            model.eval()
            imitate_statss = imitate(args, student, model, train_it, dev_it, monitor_names, extra_input)
            model.load_state_dict(student.state_dict())
            if args.save_imitate_stats:
                teacher_stats = get_imitate_stats(args, model, dev_it, monitor_names, extra_input)
