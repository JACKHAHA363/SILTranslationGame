...
mem = 16
hours = 24
slurm_dir = home_dir + "180606_3" if not interactive else ""
if not interactive and not os.path.exists(slurm_dir):
    os.mkdir(slurm_dir)
event_path = "event_180606_3"
model_path = "model_180606_3"
log_path = "log_180606_3"
decoding_path = "decoding_180606_3"

coeffs = [ (1.0, 10.0, 0.01), # 2
           (1.0, 100.0, 0.01),
         ]

msg_len_ratios = [1.0, 0.75, 1.5, 0.5, 0.25] # 5
        ...
        for (lr_anneal, linear_steps) in lr_anneals:
            for lr in lrs:
                for cpt_iter in cpt_iters:
                    ...
                    if cpt_iter == 0:
                        cmd += " --load_fr_en_from None --load_en_de_from None "
